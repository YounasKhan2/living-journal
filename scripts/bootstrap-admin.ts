...
if (!connectionString) {
  throw new Error('DATABASE_URL is required to bootstrap an admin')
}
...
function promptHidden(label: string) {
  if (!input.isTTY || !output.isTTY || typeof input.setRawMode !== 'function') {
    throw new Error('Admin bootstrap requires an interactive TTY so the password can be entered securely.')
  }
...
async function main() {
  ...
  if (!name || name.length > 120) {
    throw new Error('Admin name must be between 1 and 120 characters.')
  }
  ...
  if (existing) {
    throw new Error(`A user already exists for ${email}. Bootstrap refuses to overwrite existing accounts.`)
  }
