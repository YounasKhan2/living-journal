...
export function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>('.briefing-card__media img, .editorial-panel__media img').forEach(image => {
        gsap.fromTo(
          image,
          { scale: 1.075 },
          {
      ...
      // Keep editorial panels cinematic without pinning the entire page.
      // This avoids ScrollTrigger layout corruption during React StrictMode remounts.
      gsap.utils.toArray<HTMLElement>('.editorial-panel').forEach((panel, index) => {
        gsap.from(panel, {
          y: index === 0 ? 0 : 70,
  ...
  if (!featured) {
    return <main className="empty-page page-gutter page-top"><h1>No published stories yet.</h1></main>
  }
  ...
  return <main ref={root} className="home-canvas">
    ...
    <BriefingGrid posts={latest}/>
    <EditorialRail posts={publishedPosts.slice(0, 4)}/>
    <Trending posts={publishedPosts.filter(post => post.trending).slice(0, 5)}/>
    <NewsletterBand/>
  </main>
}
